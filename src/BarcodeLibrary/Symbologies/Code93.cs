...
/// Code 93 encoding.
/// </summary>
internal class Code93 : BarcodeEncoding, IBarcode
{
    ...
    private void InitCode93()
    {
        ...
        C93_Code.Rows.Add(new object[] { "43", "(", "100100110" }); // TODO: insert the correct character here
        C93_Code.Rows.Add(new object[] { "44", ")", "111011010" }); // TODO: insert the correct character here
        C93_Code.Rows.Add(new object[] { "45", "#", "111010110" }); // TODO: insert the correct character here
        C93_Code.Rows.Add(new object[] { "46", "@", "100110010" }); // TODO: insert the correct character here
        C93_Code.Rows.Add(new object[] { "-", "*", "101011110" });
    }

    /// <summary>
    /// Adds the C and K check digits to the input string according to the Code 93 specification.
    /// </summary>
    /// <param name="input">The input string to which check digits will be added.</param>
    /// <returns>The string with check digits added.</returns>
    private string AddCheckDigits(string input)
    {
        ...
        {
            sum += aryCWeights[i] * int.Parse(C93_Code.Select("Character = '" + input[i] + "'")[0]["Value"].ToString());
        }
        ...
        for (var i = 0; i < input.Length; i++)
        {
            sum += aryKWeights[i] * int.Parse(C93_Code.Select("Character = '" + input[i] + "'")[0]["Value"].ToString());
        }
