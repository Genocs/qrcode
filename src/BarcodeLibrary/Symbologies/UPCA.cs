...
///  UPC-A encoding.
/// </summary>
internal class UPCA : BarcodeEncoding, IBarcode
{
    private static readonly string[] UpcCodesA = { "0001101", "0011001", "0010011", "0111101", "0100011", "0110001", "0101111", "0111011", "0110111", "0001011" };
    private static readonly string[] UpcCodesB = { "1110010", "1100110", "1101100", "1000010", "1011100", "1001110", "1010000", "1000100", "1001000", "1110100" };
    private static readonly Hashtable CountryCodes = new();
    ...
    protected override string Encode()
    {
        ...
        // First number
        result += UpcCodesA[int.Parse(RawData[0].ToString())];

        // Second (group) of numbers
        int pos = 0;
        while (pos < 5)
        {
            result += UpcCodesA[int.Parse(RawData[pos + 1].ToString())];
            pos++;
        }
        ...
        while (pos < 5)
        {
            result += UpcCodesB[int.Parse(RawData[(pos++) + 6].ToString())];
        }

        // Forth
        result += UpcCodesB[int.Parse(RawData[RawData.Length - 1].ToString())];

        // Add ending guard bars
        result += "101";

        // Get the manufacturer assigning country
        string twodigitCode = $"0{RawData[..1]}";

        try
        {
            _countryAssigningManufacturerCode = CountryCodes[twodigitCode].ToString();
        }
        catch
        {
    ...
    private void CheckDigit()
    {
        try
        {
            string rawDataHolder = RawData[..11];
            ...
            RawData = rawDataHolder + checkSum;
        }
        catch
        {
