...
public class QRCodeGenerator
{
    ...
    public static QRCodeData CreateQrCode(Payload payload)
    ...
    /// <exception cref="Exceptions.DataTooLongException">Thrown when the payload is too big to be encoded in a QR code.</exception>
    /// <returns>Returns the raw QR code data which can be used for rendering.</returns>
    public static QRCodeData CreateQrCode(Payload payload, ECCLevel eccLevel)
    ...
    /// <exception cref="Exceptions.DataTooLongException">Thrown when the payload is too big to be encoded in a QR code.</exception>
    /// <returns>Returns the raw QR code data which can be used for rendering.</returns>
    public static QRCodeData CreateQrCode(string plainText, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1)
    ...
    public static QRCodeData GenerateQrCode(Payload payload, ECCLevel eccLevel)
        => GenerateQrCode(payload.ToString(), eccLevel, false, false, payload.EciMode, payload.Version);
    ...
    public static QRCodeData GenerateQrCode(string plainText, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1)
    {
        ...
        string codedText = PlainTextToBinary(plainText, encoding, eciMode, utf8BOM, forceUtf8);
        int dataInputLength = GetDataLength(encoding, plainText, codedText, forceUtf8);
        ...
        string countIndicator = DecToBin(dataInputLength, GetCountIndicatorLength(version, encoding));
    ...
    private static class ModulePlacer
    {
        ...
        public static void PlaceVersion(ref QRCodeData qrCode, string versionStr)
        {
            ...
            string reverseString = ReverseString(versionStr);

            for (int x = 0; x < 6; x++)
            {
                for (int y = 0; y < 3; y++)
                {
                    qrCode.ModuleMatrix[y + size - 11][x] = reverseString[x * 3 + y] == '1';
                    qrCode.ModuleMatrix[x][y + size - 11] = reverseString[x * 3 + y] == '1';
                }
            }
        }
        ...
        {
            int size = qrCode.ModuleMatrix.Count;
            string fStr = ReverseString(formatStr);
            int[,] modules = new[,] {
                { 8, 0, size - 1, 8 },
                { 8, 1, size - 2, 8 },
                { 8, 2, size - 3, 8 },
    ...
    private static bool IsInRange(char c, char min, char max)
        => unchecked((uint)(c - min) <= (uint)(max - min));
    ...
    private static string PlainTextToBinaryByte(string plainText, EciMode eciMode, bool utf8BOM, bool forceUtf8)
    {
        ...
        else
        {
            codeBytes = eciMode switch
            {
                ...
                EciMode.Iso8859_2 => Encoding.GetEncoding("ISO-8859-2").GetBytes(ConvertToIso8859(plainText, "ISO-8859-2")),
